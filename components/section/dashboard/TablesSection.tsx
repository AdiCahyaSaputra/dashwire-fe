...
import EmptyStateSection from 'components/section/global/EmptyStateSection'
  ...
  return (
    <>
      {tableData.length ? (
        ...
        <EmptyStateSection message={`There's No Data In This Table!`} action={{
          label: 'Try Create New One?',
          url: `/dashboard/${router.query?.id}/create`
        }} />
      )}
    </>
  )
